...
class _GameScreenState extends State<GameScreen>
  ...
  set _nextLifeAt(DateTime? val) => GameStats.nextLifeAt = val;
  Timer? _lifeTimer;
  int get _lives => GameStats.lives;
  set _lives(int val) => GameStats.lives = val;
  int get _gold => GameStats.gold;
  set _gold(int val) => GameStats.gold = val;
  ...
  bool get _isStarLevel => _levelIndex == 84 || _levelIndex == 86 || _levelIndex == 93;

  List<List<int>> get _starCenters {
    if (_levelIndex == 88) {
      return const [[56, 12], [56, 30], [56, 48]];
    } else if (_levelIndex == 93) {
      return const [[6, 6], [6, 52], [53, 6], [53, 52], [55, 29]];
    } else {
  ...
  bool _isLineOfSightBlockedByChain(MotorSide side, int index) {
    ...
    if (_levelIndex == 90) {
      if (side == MotorSide.left) {
        activeIndices = [for (int i = 0; i < 25; i++) i];
      } else {
        activeIndices = [for (int i = 25; i < 50; i++) i];
      }
    } else {
    ...
    double getLinkPos(int i) {
      if (_levelIndex == 90) {
        final int linkIdx = (side == MotorSide.left) ? i : (i - 25);
        return 3.5 + (linkIdx / 24.0) * 53.0;
      }
  ...
  void _resetPrototype() {
    ...
    if (_levelIndex == 52) {
      ...
      // Level 91 (NASCAR Pixel): Active chains: Left (0..24) and Right (25..49) sides, pre-break remaining 22 links (50..71).
      for (int i = 0; i < 72; i++) {
        if (i >= 50) {
          _chainLinkHits[i] = 20;
          _brokenLinks.add(i);
