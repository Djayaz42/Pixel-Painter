...
void main() {
  ...
  final colorMap = {
    ...
    15: [108, 122, 137],  // 0xFF6C7A89
    16: [44, 94, 138],    // 0xFF2C5E8A
  ...
  for (int y = 0; y < size; y++) {
    ...
    for (int x = 0; x < size; x++) {
      if (isBg[y][x]) {
        ...
        // Simplify colors to group similar ones
        if (closestId == 24 || closestId == 25 || closestId == 26 || closestId == 7) {
          closestId = 25; // Group to light teal
        } else if (closestId == 3 || closestId == 21 || closestId == 22 || closestId == 23) {
          closestId = 21; // Group to dark teal/green
        } else if (closestId == 27 || closestId == 29 || closestId == 41 || closestId == 4) {
          closestId = 4;  // Group to gold/yellow
        } else if (closestId == 6 || closestId == 30) {
          closestId = 6;  // Group to orange
        } else if (closestId == 11 || closestId == 20 || closestId == 31 || closestId == 42) {
          closestId = 31; // Group to light parchment
        } else if (closestId == 32 || closestId == 34 || closestId == 37 || closestId == 40) {
          closestId = 34; // Group to reddish brown
        } else if (closestId == 14 || closestId == 38 || closestId == 39) {
          closestId = 14; // Group to dark brown
        } else if (closestId == 13 || closestId == 33 || closestId == 35) {
          closestId = 13; // Group to light gray/brown
        }
